...
<div class="container">
	<div class="page-header">
		<div><h4>여행 문의 <a style="text-decoration: none">${map.count }</a></h4></div>
	</div>
	...
	<div class="con">
		...
		<table class="table table-hover">
			...
			<tbody>
				...
				<tr>
					<td>${row.bno}</td>		
					<td><a href="${path}/contact/contact_view?bno=${row.bno}&pageNum=${pageMaker.cri.pageNum }&amount=${pageMaker.cri.amount }">${row.title}</a></td>		
					<td>${row.writer}</td>		
		...
		<div class="row">
			<div class="form-group">
				...
				<div class="pull-right">
					<a href="${path}/contact/contact_write"><button type="button" class="btn btn-default">문의 등록</button></a>
				</div>
			</div>
		</div>
		</c:if>
		...
		<form name="form1" method="post" action="${path }/contact/contact_list" class="form-inline">
		<div class="form-group">
...
<script>
	...
	$(".pageInfo_btn a").on("click", function(e){
	      ...
	      moveForm.attr("action", "${path}/contact/contact_list");
	      moveForm.submit();
	});
</script>
</html>